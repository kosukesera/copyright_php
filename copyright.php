...
//$exp === 1 (正式記法)
function cprt_exp1($crid, $workttl, $author, $format, $workurl, $morepermurl, $f1, $f2, $orgurl){
...
//CCの時
	...
	//著者名
	$code .= "<br /><span xmlns:cc=\"http://creativecommons.org/ns#\" property=\"cc:attributionName\">{$author}</span>による作品";
	//作品名
	$code .= "『<span xmlns:dct=\"http://purl.org/dc/terms/\" {$fomat1} property="dct:title" {$fomat2}>{$workttl}</span>』は";
	//ライセンス
...
//$exp === ２ (英略記法)
function cprt_exp2($crid, $workttl, $author, $format, $workurl, $morepermurl, $f1, $f2, $orgurl){
//元になる作品があった場合
if($orgurl !===  ""){
...
//CC
	...
	$code = "<a rel=\"license\" href=\"http://creativecommons.org/licenses/{$cpid}/{$ccver}/\"><img alt=\"クリエイティブ・コモンズ・ライセンス\" style=\"border-width:0\" src=\"https://i.creativecommons.org/l/{$cpid}/{$ccver}/{$ccbt}\" /></a>";
	//作品名
	$code .= "『<span xmlns:dct=\"http://purl.org/dc/terms/\" {$fomat1} property="dct:title" {$fomat2}>{$workttl}</span>』";
	//著者名
	$code .= "by <span xmlns:cc=\"http://creativecommons.org/ns#\" property=\"cc:attributionName\">{$author}</span> {$perm}";
}
return $code;
//関数終わり
};
...
function cprt(){
   ...
   if($num >= 8){
	...
	//"-1", "0", "1", "2", "3", "4", "5", "6", "9"
	$workttl = func_get_arg(2); //作品タイトル
	$author = func_get_arg(3);	//著作権者
	$format = func_get_arg(4);	//フォーマット
	$workurl = func_get_arg(5);		//作品のクレジット用URL
	$morepermurl = func_get_arg(6);	//許可外の使用を希望する際に確認すべきURL
    //入力変数（補足）
    if($num = 7){
    $orgurl = func_get_arg(7)	//元になった作品のURL(改変の場合等)
    }else{
    $orgurl = "";
    }
    ...
    $f2 = "rel=\"dct:type\"";
    if($exp === "1"){
    	return cprt_exp1($crid, $workttl, $author, $format, $workurl, $morepermurl, $f1, $f2, $orgurl);
    }elseif($exp === "2"){
    	return cprt_exp2($crid, $workttl, $author, $format, $workurl, $morepermurl, $f1, $f2, $orgurl);
    }else{  
